...
import { verifyGlobalAuditLedger } from "@/lib/cases/repository";
...
import { getFindingExportData, getFindingExportProvenance } from "@/lib/findings/repository";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!findingExportFormats.includes(formatValue as (typeof findingExportFormats)[number])) {
    return jsonResponse({ error: "Export format must be CSV, JSON, or bundle." }, 400, requestId);
  }
  ...
  try {
    ...
    if (format === "bundle") {
      if (!payload.summary.reviewed) {
        return jsonResponse({ error: "Review at least one finding before creating a hand-off bundle." }, 409, requestId);
      }
      const verification = await verifyGlobalAuditLedger();
      if (verification.status !== "verified") {
        return jsonResponse({ error: "The audit ledger must be verified before creating a hand-off bundle." }, 409, requestId);
      }
      const provenance = await getFindingExportProvenance(parsedId.data);
      const bundle = buildReviewedFindingBundle(payload, provenance, verification);
      requestLog("info", "findings.exported", requestId, {
        caseId: parsedId.data,
        format,
        reviewedFindings: payload.summary.reviewed,
        referencedSources: bundle.manifest.summary.referencedSources,
      });
      return new Response(Uint8Array.from(bundle.bytes).buffer, {
        status: 200,
        headers: {
          "Cache-Control": "private, no-store, max-age=0",
          "Content-Disposition": `attachment; filename="traceframe-case-${parsedId.data}-reviewed-findings.zip"`,
