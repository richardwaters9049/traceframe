...
export async function getCaseWorkspace(caseId: string) {
  const sql = getDatabaseClient();
  const [caseRows, auditRows, verification, sources, findingCollection] = await Promise.all([
