...
export async function getCaseWorkspace(caseId: string) {
  const sql = getDatabaseClient();
  const [caseRows, auditRows, verification] = await Promise.all([
  ...
  return { case: serialiseCase(caseRows[0]), auditEvents: auditRows.map(serialiseAuditEvent), verification };
}
