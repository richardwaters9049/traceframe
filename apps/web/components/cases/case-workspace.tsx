...
export function CaseWorkspace({ workspace }: { workspace: CaseWorkspaceRecord }) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
      ...
      <div className="ui-label mt-10 flex gap-7 border-b border-white/[0.065] text-[#98A2B2]"><button className="border-b-2 border-[#7C8DFF] pb-3 text-[#E3E7ED]">Analysis</button><button className="pb-3">Sources</button><button className="pb-3">Relationships</button></div>
      <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="min-h-[28rem] rounded-[1.5rem] border border-white/[0.07] bg-[#10131A] p-5 sm:p-7"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-[#7C8DFF]/10 text-[#91A0FF]"><FileText className="size-5" /></span><div><h2 className="ui-section-title">Analysis workspace</h2><p className="ui-eyebrow mt-1 text-[#8F99AA]">Case established</p></div></div><div className="mt-7 grid min-h-[20rem] place-items-center rounded-2xl border border-dashed border-white/[0.075] bg-white/[0.012] p-8 text-center"><div><span className="mx-auto grid size-11 place-items-center rounded-full bg-[#58D6C7]/[0.06] text-[#69DCCE]"><Check className="size-5" /></span><h3 className="ui-section-title mt-4">Ready for source material</h3><p className="ui-meta mt-2 text-[#AAB3C1]">Evidence ingestion will appear here.</p></div></div></div>
        <aside className="h-fit rounded-[1.5rem] border border-white/[0.07] bg-[#10131A] p-5"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-[#58D6C7]/[0.07] text-[#6CDDD0]"><Fingerprint className="size-5" /></span><div><h2 className="ui-section-title">Audit trail</h2><p className="ui-eyebrow mt-1 text-[#8F99AA]">Verified history</p></div></div><div className="mt-6 space-y-5 border-l border-white/[0.07] pl-5">{auditEvents.map((event) => <article key={event.id} className="relative before:absolute before:-left-[1.43rem] before:top-1.5 before:size-2 before:rounded-full before:bg-[#7C8DFF]"><p className="ui-label">Case created</p><p className="ui-meta mt-1.5 truncate text-[#AAB3C1]">{event.actorId}</p><p className="ui-meta mt-3 font-mono text-[#8993A4]">{event.eventHash.slice(0, 16)}…</p></article>)}</div></aside>
      </div>
    </section>
  );
}
