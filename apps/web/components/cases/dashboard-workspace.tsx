...
export function DashboardWorkspace({ cases }: { cases: CaseRecord[] }) {
  const { openNewCase } = useWorkspaceUI();
  const urgentCases = cases.filter((record) => record.priority === "critical" || record.priority === "high").length;
      ...
      >
        <motion.header variants={{ hidden: { opacity: 0, y: 14 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.45, ease: "easeOut" }} className="grid min-w-0 gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex min-w-0 flex-wrap items-baseline gap-x-4 gap-y-1">
              ...
              <span className="ui-meta text-[#8A94A6]">{cases.length} active {cases.length === 1 ? "case" : "cases"}</span>
            </div>
          </div>
          <button
            type="button"
            onClick={openNewCase}
            className="inline-flex h-10 w-fit items-center gap-2 rounded-xl bg-[#7C8DFF] px-4 text-sm font-semibold tracking-[0.03em] text-[#090B10] transition-colors hover:bg-[#93A1FF]"
          >
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 18 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.5, ease: "easeOut" }} className="grid min-h-0 min-w-0 items-stretch gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
          ...
          <aside className="grid min-w-0 auto-rows-fr gap-4 sm:grid-cols-3 lg:grid-cols-1 lg:grid-rows-3">
            <motion.div whileHover={{ y: -3, borderColor: "rgba(124,141,255,0.22)" }} transition={{ type: "spring", stiffness: 320, damping: 24 }} className="flex min-h-0 min-w-0 flex-col justify-evenly rounded-[1.35rem] border border-white/[0.07] bg-[#10131A] p-5">
              <span className="grid size-14 place-items-center rounded-2xl bg-[#7C8DFF]/12 text-[#A8B3FF]">
                <FolderKanban className="size-7" aria-hidden="true" />
              </span>
              <p className="min-w-0 pl-[13px] pt-[10px] text-3xl font-medium tracking-[-0.02em]">{cases.length}</p>
              <p className="ui-meta min-w-0 pl-[13px] pt-[10px] text-[#C0C7D2]">Open records</p>
            </motion.div>

            <motion.div whileHover={{ y: -3, borderColor: "rgba(253,224,71,0.18)" }} transition={{ type: "spring", stiffness: 320, damping: 24 }} className="flex min-h-0 min-w-0 flex-col justify-evenly rounded-[1.35rem] border border-white/[0.07] bg-[#10131A] p-5">
              ...
              <p className="min-w-0 pl-[13px] pt-[10px] text-3xl font-medium tracking-[-0.02em]">{urgentCases}</p>
            ...
            <motion.div whileHover={{ y: -3, borderColor: "rgba(88,214,199,0.25)" }} transition={{ type: "spring", stiffness: 320, damping: 24 }} className="flex min-h-0 min-w-0 flex-col justify-evenly rounded-[1.35rem] border border-[#58D6C7]/12 bg-[#58D6C7]/[0.035] p-5">
              ...
              <p className="ui-section-title min-w-0 pl-[13px] pt-[10px] text-[#F0F4F7]">Chain verified</p>
              <p className="ui-meta min-w-0 break-words pl-[13px] pt-[10px] text-[#C0C9D3]">Audit writes remain linked.</p>
            </motion.div>
          </aside>
        </motion.div>
      </motion.section>
